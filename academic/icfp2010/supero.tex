...
\todo{perhaps switch to tagsoup as the openning example, so I can come back to it? yes - if it works}
...
The supercompiler takes a source program, and generates a target program. Functions in these programs are distinct -- target expressions cannot refer to source functions. The source and target program are semantically identical, but it is hoped the target program runs faster. We use the type |Env| to represent a mapping from function names to expressions. We can think of a root lambda expression as a function, or as an \textit{open expression} whose free variables are given by the lambda.
...
The conditions in \S\ref{sec:eval_split_lambda} hold.
...
We originally had binary application, but that doesn't work well with primitives. Also harder to move partial application.
...
\subsubsection{Splitting Heuristic}
...
\begin{code}
root i = map square (iterate (+1) 1) !! i
    where square x = x * x
\end{code}

Let's consider this program. If we compile and run for n = 400000 we get GHC taking 0.149 seconds, and Supero taking 0.011 seconds. With stream fusion it gets reduced to 1.25. Unfortunately GHC stack overflows if it goes higher.

A few things to note:

Supercompilation has eliminated all the lists from this program, and yet has no knowledge of lists.

While stream fusion is powerful, it isn't a panacea.

The strictness analysis works much better with the lists gone - GHC is remarkably efficient at creating lists.

There is no rule map f x !! i ==> f (x !! i), but there probably should be. However, all these rely on people using specific functions, and people predicting patterns. In this case I don't think it matter as both GHC and Stream fusion manage to fuse the map/iterate, but in general it would help.

GHC reduces it all to:
...
To get this result supero had to do specialisation of the iterate function and the map function. GHC can do this by relying on some form of pushing things down and making them non-recursive, but it sucks.

Note that it doesn't square each element, it has in effect inverted the flow so that only the final element is squared.

Here are some examples of supercompiled functions, with a little bit of commentary.


While the example is fake, the results are real. We go 21 times faster than GHC.

We also compared to stream fusion, which resulted in an improvement of n. We deliberately choose to 
...
Note that the totally different termination criteria and vastly different other bits lead to a generalisation/split strategy that is entirely different, and also very simple.
