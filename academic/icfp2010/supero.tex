...
\todo{we must first eliminate all environment variables that point directly, otherwise they look equal under $\lhd$ and make the termination bork}

Thoughts:

An application is both an application, and a variable. Perhaps we need to track the first variable in an application too? Or make the rule that all environment variables are bound separately? Certainly something subtle going on, which is caused by the vector application. Perhaps we need all functions bound separately?

Reintroduce Fun, for something that is not bound locally. 

Hello Mike and Max. I appreciate you taking a look at my paper. The paper is not yet finished, I'm still working on some of the performance numbers -- I've only got them in preliminary form so far. I've done sections 2 (method), 3 (iterate example) and 6 (related work). They make up most of the paper. The intro is enough to read to get an introduction, but I might switch it round a fair bit. Any comments, particularly on those bits would be most appreciated. What do you think of the title? I'm never very good at titles. I'm aiming for this to be the paper that is simple and explicit enough that either of you feel you could implement it in a day, and that there are no hidden corners that contain magic. I know my previous paper didn't have that property (mainly because I tried to implement it a year later!). I'm hoping to have a version with all the performance numbers on Monday, then spend until Friday (April 2nd) polishing the text.
...
\todo{The section name is wrong, but I don't know what it should be...}
