...
\begin{abstract}
Haskell is a functional language, with features such as higher order functions and lazy evaluation, which allow succinct programs. These high-level features present many challenges for optimising compilers. We report practical experiments using novel variants of \textit{supercompilation}, with special attention to let bindings and the generalisation technique. Results include speed improvements of up to 55\% when our supercompiler is used in conjunction with GHC.
\end{abstract}
...
split(c) = (c, [])
split(f) = (f, [])
split(x ys_) = (bullet bullet, x:ys_)
split(\vs_ -> x) = (\vs_ -> bullet, x)
split(let v = x in y) = (let v = bullet in bullet, [x,y])
split(case x of {p_1 -> y_1 ; ... ; p_n -> y_n}) = (case x of {p_1 -> bullet ; ... ; p_n -> bullet, [y_1, ... ,y_n])
\end{code}
\figend
\caption{The split function, returning a shell and all subexpressions.}
\label{fig:core}
\end{fig}
...
The spine of an expression is an expression without its immediate subexpressions.
...
Taking the second example, |repeat| can be inlined indefinately. However, by unfolding the |const| we produce |let x = repeat 1 in 0 : map f x|. Since |x| is only used once we may substitute it to produce |(0 : map (+1) (repeat 1))|, which may be deforested appropriately.
...
&
\[\frac{\sigma_1 \cong \sigma_2,
        s_1 \unlhd t_1, \ldots , s_n \unlhd t_n}
       {\text{couple}(\sigma_1 (s_1,\ldots,s_n), \sigma_2 (t_1,\ldots,t_n))}
\]
\end{tabular}

We use $\sigma$ to denote the spine of an expression, with $s_1,\ldots,s_n$ being its subexpressions. We test for equivalence of $\sigma_1$ and $\sigma_2$ using the $\cong$ relation, a weakened form of equality where all variables are considered equal. We terminate the supercompilation of an expression |y| if on the chain of reductions from |main| to |y| we have encountered an expression |x| such that $x \unlhd y$.
...
The homeomorhpic embedding detects the repeated occurrence of the |putStr| application, and the supercompilation of this expression is stopped. The most specific generalisation rule produces:
...
The problem is that the rule works from the top of both expressions looking for a common root of both trees. However, if the first rule applied was diving, the roots may be unrelated. Using our $\bowtie$ generalisation:
...
We exclude two benchmarks, paraffins and integrate. The paraffins benchmark makes substantial use of arrays, and we have not yet mapped the array primitives of Yhc onto those of GHC, which would be required to run the transformed result. The integrate benchmark tests arithmetic on the |Double| type, and can be optimised by Supero. Unfortunately, the resultant executables consume increasingly large amounts of memory and fail to produce an answer. We suspect the mistranslation of a primitive operation is at fault, but have yet to track down the cause.
