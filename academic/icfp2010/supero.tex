...
\item Our let expression is non-recursive. Bindings within a let expression are bound in order, and an expression on the right-hand side may not refer to variables defined later. For example, we allow |let x = y; y = C in C| but not |let x = y; y = x in C|.
...
We now define a simplified form of our Core language. When working with Core expressions we assume they are always simplified, and after constructing new expressions we always simplify them. Our simplified form requires that all variables in the top-level environment be bound to a (possibly empty) sequence of lambda expressions, followed by a let expression -- which we call the \textit{root let}.
