...
The simplifier as presented will not terminate for all programs. Take the following example:

\begin{code}
data U = MkU (U -> Bool)

e = (\u -> case u of MkU p -> p u)
    (MkU (\x -> case x of MkU y -> y x)) 
\end{code}

This program encodes recursion via a data type, and has no simplified representation. This problem has been covered 

However, in the presence of data constructors it is possible to define
(well-typed!) fix-point combinator by means of lambdas and pattern
matching only.



This problem also effects GHC, and has been described in by \citet{klyuchnikov:termination}. There are no easy changes to fix this, but you could put a numeric bound on the number of simplification steps -- however it's a corner case which is unlikely to occur in real programs.
