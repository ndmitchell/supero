...
\todo{The non-termination issue was also spotted (after the submission of my paper) by Ilya Klyuchnikov, who has written a paper "Supercompiler HOSC 1.1: proof of termination". His paper discusses the non-termination problem, and I will include a citation. The non-termination is a corner case (unlikely to occur in real programs), but should be possible to fix with a small change to the simplifier. Thank you for finding this example.}

\todo{The typing for Core programs is considered to be Hindley-Milner.}

\todo{The references to termination orderings are somewhat light, I will rectify this.}

\todo{The statement about inlining was intended to express that GHC uses inlining to move expressions together which allows them to reduce. My use of inlining in the post-processing phase is purely to reduce function call overhead. The main supercompilation phase subsumes inlining, so the techniques are not really comparable.}

\todo{Thanks for the useful feedback on the areas that are less clear. I agree that a formal AST would help, and will include one - which I think will also make the arity definition clearer.}

\todo{Thanks for the feedback on related work, I will revise the citations.}

\todo{By manager I do mean the supercompile function - manager is certainly a better name.}
