...
\todo{all v = map things can be eliminated, now Fun is gone}
...
\todo{change: remove fun everywhere, for evaluation (the only place mentioning fun) just say only evaluate it if it has a body. when passing free variables only include those bound locally (keeps things identical).}
...
supercompile env =
    assign $ flatten $ optimise env $ fromJust $ env "main"
...
-- \todo{now wrong as lambda is in the Exp}
